...
- `postman/collections/bd-bd-users-grpc.postman_collection.json` — **bd-management** `CreateBdAccount` / `UpdateBdAccount` gRPC + HTTP 验收（见 [`docs/bd-users-grpc-testing.md`](docs/bd-users-grpc-testing.md)；`npm run pm:bd-users-grpc:local`）。
