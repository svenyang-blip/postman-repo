...
- **`reports/newman-summary-zh.html`**：由 `scripts/newman-json-to-zh-html.mjs` 生成，**全文中文**、样式内联，**不依赖外网 CDN**（适合内网或 CDN 不可达环境）。
...
- `postman/collections/pm-ce-testnet-event-list.postman_collection.json` — 多文件夹用例（分页、`eventStatus`、`sortBy`、热门、`locale`、`categoryIds`、响应契约等），默认 **17** 个请求。
